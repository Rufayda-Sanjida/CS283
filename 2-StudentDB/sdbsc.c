...
{
    // TODO
    int position = id * STUDENT_RECORD_SIZE;

    if (lseek(fd, position, SEEK_SET) < 0) {
        return ERR_DB_FILE; 
    }

    int read_size = read(fd, s, STUDENT_RECORD_SIZE);
    if (read_size == -1) {
        return ERR_DB_FILE; 
    }
    
    if (read_size != STUDENT_RECORD_SIZE) {
...
{
    // TODO
    int record_offset = id * STUDENT_RECORD_SIZE;
    int current_pos = lseek(fd, record_offset, SEEK_SET);
    ...
    student_t student = {0};
    int read_status = read(fd, &student, STUDENT_RECORD_SIZE);
    ...
    current_pos = lseek(fd, record_offset, SEEK_SET);
    if (current_pos < 0) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }

    int write_status = write(fd, &student, STUDENT_RECORD_SIZE);
    if (write_status < 0) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
...
{
    student_t record;
    int search_result = get_student(fd, id, &record);
    
    if (search_result == SRCH_NOT_FOUND){
    ...
    int remove_status = write(fd, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE);
...
int count_db_records(int fd)
{
    ...
    for (int student_id = MIN_STD_ID; student_id <= MAX_STD_ID; student_id++) {
        int record_offset = student_id * STUDENT_RECORD_SIZE;
        int current_pos = lseek(fd, record_offset, SEEK_SET);
        ...
        student_t student_record = {0};
        int read_status = read(fd, &student_record, STUDENT_RECORD_SIZE);
        ...
        if (memcmp(&student_record, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            record_count++;
        }
    }
...
int print_db(int fd)
{
    ...
    for (int student_id = MIN_STD_ID; student_id <= MAX_STD_ID; student_id++) {
        int record_offset = student_id * STUDENT_RECORD_SIZE;
        int seek_pos = lseek(fd, record_offset, SEEK_SET);
        ...
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE)!= 0) { //empty 
            if (!header) {
                printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST NAME", "LAST_NAME", "GPA");
                header = true;
            }
            
            float gpa_value = ((float)student.gpa)/ 100.0;

            printf(STUDENT_PRINT_FMT_STRING, student.id, student.fname, student.lname, gpa_value);
            total_records++;
        }
    }
...
void print_student(student_t *s)
{
    ...
    printf(STUDENT_PRINT_FMT_STRING, s->id, s->fname, s->lname, gpa_value);
}
...
int compress_db(int fd)
{
    ...
    for (int student_id = MIN_STD_ID; student_id <= MAX_STD_ID; student_id++) {
        student_t student;
        int result_code = get_student(fd, student_id, &student);

        if (result_code == NO_ERROR) {
            int write_result = write(temp_fd, &student, STUDENT_RECORD_SIZE);
            if (write_result != STUDENT_RECORD_SIZE) {
                printf(M_ERR_DB_WRITE);
                close(temp_fd);
                return ERR_DB_FILE;
            }
        } else if (result_code == ERR_DB_FILE) {
            printf(M_ERR_DB_READ);
            close(temp_fd);
            return ERR_DB_FILE;
        }
